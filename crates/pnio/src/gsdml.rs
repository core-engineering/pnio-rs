...
pub struct GsdmlMeta {
    /// `VendorName` (rendered under `DeviceIdentity` and in the DAP `ModuleInfo`); also cleaned to `[A-Za-z0-9]` for the file
    /// name — see [`file_name`].
    pub vendor_name: String,
    /// `ProductFamily` text; also cleaned to `[A-Za-z0-9]` for the file name — see
    /// [`file_name`].
    pub product_family: String,
    /// `InfoText` of the device (the GSDML's free-text description).
