...
# cmdline, so `pgrep -f` never matches; match the (15-char, comm-truncated) thread name
# instead, e.g. "irq/131-eno2-Tx" for the real thread name "irq/131-eno2-TxRx-0".
for pid in $(pgrep "^irq/[0-9]+-${PLC_IF}-Tx" || true); do
...
for pid in $(pgrep "^irq/[0-9]+-${PLC_IF}-Tx" || true); do echo "irqthread: pid $pid $(chrt -p "$pid" | tr '\n' ' ')"; done
