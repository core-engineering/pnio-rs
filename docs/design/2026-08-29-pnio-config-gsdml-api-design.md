...
| GSDML | V2.4 schema (the one TIA accepted for rt-labs), text template, `PNIO_Version="V2.3"`, `StartupMode="Advanced"`, RT class 1 only, `MinDeviceInterval` from config (32 or 16), `SupportedProtocols=""` | What the bench already proved; nothing declared that the device cannot do — TIA's GSD checker mandates LLDP/PTP-DCP-boundary/ResetToFactory/CertificationInfo claims from `PNIO_Version >= "V2.31"`, and the device implements none of them, so the DAP declares `PNIO_Version="V2.3"` (still allows `StartupMode="Advanced"`) and the interface submodule declares `SupportedProtocols=""` (neither SNMP nor LLDP; the v2.4 XSD makes the attribute required but its `TokenListT` type allows an empty list, so omitting it fails validation) |
...
```
`ConfigError` (thiserror, `PartialEq`): `SlotZeroReserved`, `DuplicateSlot(u16)`, `EmptySubmodule(u16)`, `NoSubmodule`, `TooLong { slot, bytes, max: 1440 }` (per-submodule, one direction), `BadStationName(String)` (DCP rule: lowercase `a-z0-9-.` labels, ≤ 240 bytes, no leading/trailing `-`, not all digits, not a reserved `port-xyz`/`port-xyz-abcde` auto-port name), `BadInterval(u16)` (allowed: 16, 32 only — 8 would need a busy-poll device we don't implement, 64/128 are not send clocks this crate tests or declares in the GSDML), `BadIdentity` (`vendor_id == 0`), `TooLongTotal { direction: Direction, bytes: u32, max: 1440 }` (the *total* C-SDU of one direction across every submodule, computed the way `rt::Layout` lays the CR out: 3 bytes of DAP IOPS/IOCS + Σ over submodules with data in that direction of `(len + 1)` + one IOCS byte per submodule with data only in the other direction).
...
- `input_cr_len(&self) -> u16` / `output_cr_len(&self) -> u16`: the Input/Output CR C-SDU length including the IOPS/IOCS bytes, as the controller (and `gsdml::render`'s `IOConfigData`) will compute it — the same `cr_lengths` sum `TooLongTotal` guards, ≤ 1440 by construction.
...
- `ApplicationProcess/DeviceAccessPointList/DeviceAccessPointItem ID="DAP1" PNIO_Version="V2.3" PhysicalSlots="0..N" ModuleIdentNumber="0x00000001" MinDeviceInterval="<cfg>" DNS_CompatibleName="<station>" FixedInSlots="0" ObjectUUID_LocalIndex="1"`, `ModuleInfo` (name/order number), `SubslotList` (`0x8000` interface, `0x8001` port), `IOConfigData MaxInputLength/MaxOutputLength` = the Input/Output CR C-SDU lengths including the IOPS/IOCS bytes ([`DeviceConfig::input_cr_len`]/[`DeviceConfig::output_cr_len`], the same accounting `config::check_total_csdu` guards — 3-byte DAP IOPS/IOCS + one `(data_len + 1)` per submodule with data in that direction + one IOCS byte per submodule with data only in the other direction), not the plain per-direction data sums; `MaxDataLength` = their sum (present, optional, in the v2.4 XSD), `UseableModules` with one `ModuleItemRef ModuleItemTarget="M<n>" AllowedInSlots="<n>"` per slot, `VirtualSubmoduleList/VirtualSubmoduleItem ID="DAP1_SM" SubmoduleIdentNumber="0x00000001"` with empty `IOData`, `SystemDefinedSubmoduleList`: `InterfaceSubmoduleItem SubslotNumber="32768" SubmoduleIdentNumber="0x00008000" SupportedRT_Classes="RT_CLASS_1" SupportedProtocols=""` (neither SNMP nor LLDP is implemented; the attribute is `use="required"` in the v2.4 XSD, but its `TokenListT` type allows an empty list) + `ApplicationRelations StartupMode="Advanced"` + `TimingProperties SendClock="32" ReductionRatio="1 2 4 8 16 32 64 128 256 512"` (`SendClock="16 32"` when `min_device_interval == 16`), `PortSubmoduleItem SubslotNumber="32769" SubmoduleIdentNumber="0x00008001" MAUTypes="16"`. `PNIO_Version="V2.3"`, not the reference file's `"V2.4"`: TIA's GSD checker mandates `CertificationInfo`/`LLDP_NoD_Supported`/`ResetToFactoryModes`/`PTP_BoundarySupported`/`DCP_BoundarySupported` claims from `PNIO_Version >= "V2.31"`, none of which the device implements; V2.3 is the last version without those mandates and still allows `StartupMode="Advanced"`.
...
- `ExternalTextList/PrimaryLanguage`: one `Text TextId="…" Value="…"` per referenced id (`M<n>_Name`, `M<n>_Info`, `M<n>_<In|Out><i>` per scalar `DataItem`, `M<n>_<In|Out><first>_bits` per BOOL-group `DataItem` (`first` = the group's first field index), bit ids `M<n>_<In|Out><i>_b` per `BitDataItem` — `i` is the bit field's own declaration-order index, already unique, so no extra `_b<j>` suffix is needed).
